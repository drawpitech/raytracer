...
Viewport::Viewport(
    maths::Rectangle3<double> rect, maths::Vector2<std::size_t> resolution,
...
maths::Ray3<double> Viewport::ray(maths::Point2<double> point) const {
    if (point.x() < 0 || point.x() > static_cast<double>(_size.x()) + 1 ||
        point.y() < 0 || point.y() > static_cast<double>(_size.y()) + 1) {
    ...
    return {
        _position,
        _position.vectorTo(
            _origin + _down * (point.y() + 0.5) + _right * (point.x() + 0.5))};
}
}  // namespace rtx::basics
