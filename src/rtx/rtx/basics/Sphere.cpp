...
std::optional<core::Hitpoint> Sphere::hit(
    const maths::Ray3<double>& ray) const {
    ...
    const auto sens = _radius > oc.norm() ? -1 : 1;

    const maths::Point3 point = ray.origin() + ray.direction() * t;
    return core::Hitpoint{
        ray, t, point, (_center.vectorTo(point) * sens).normalized()};
}
